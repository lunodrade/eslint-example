...
Route::get("/ASPAS DUPLAS", function () {  return Auth::user()    ? redirect()->action('HomeController@index')  : view('auth.login');
});
...
Route::get('company/{id}/containers', function ($id) {
    if (Gate::allows('migra')) 
    {
        ...
        return json_encode(
            App\Container::onlyCompany(Auth::user()->company_id)->where('company_service_id', $id)    ->whereNull('service_order_id')
                ->pluck('name', 'id')
        );
    }
});
